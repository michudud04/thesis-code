function [adj,lengths,thicknesses,nodes,distances] = assymetricWattsStrogatz(adjacencyMatrix, lengths, thicknesses,nodes, distances, p,r,min_component_size)
    ...
    for i = 1:N  % Iterate over all nodes
        ...
        for j = 1:length(neighbors)
            if rand < p  % Rewire with probability p
                ...
                %change this line to let the network disconnect upon
                %rewiring
                if connected == false
                    %can't rewire as that would disconnect the network
                    adj(i, neighbors(j)) = 1;
                elseif rand < r
